...
def testar_sistema_historico():
    ...
    try:
        ...
        # Comando dismiss
        history.record_command("dismiss", {
            "data_hora": "15/01/2024 15:45:10"
        }, True)

        # Comando processar
        history.record_command("processar", {
            "force": True,
            "entry": None,
            "backup": True
        }, True)
        ...
        if stats["total_commands"] != 4:
            print(f"❌ Estatísticas incorretas: {stats['total_commands']} (esperado: 4)")
            return False

        if stats["successful_commands"] != 3:
            print(f"❌ Comandos bem-sucedidos incorretos: {stats['successful_commands']} (esperado: 3)")
            return False

        if stats["failed_commands"] != 1:
            print(f"❌ Comandos com falha incorretos: {stats['failed_commands']} (esperado: 1)")
            return False
...
def executar_todos_testes():
    ...
    nomes_testes = ["OCR Individual", "Funções ChatGPT", "Processamento Completo", "Histórico"]
